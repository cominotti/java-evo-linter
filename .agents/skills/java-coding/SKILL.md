...
name: java-coding
description: >-
  Mandatory default skill for any Java-related work. Trigger this skill whenever the user asks to
  write, edit, refactor, review, debug, test, optimize, or explain Java code or Java build/lint
  behavior. This includes requests involving .java files, Maven (pom.xml, plugins, dependencies,
  compiler release, surefire/failsafe), CI for JDK versions, Checkstyle, PMD, SpotBugs, Error
  Prone, NullAway, OpenRewrite, var usage policy, records/enums/interfaces/classes, annotations,
  exceptions, concurrency, streams, collections, and Java CLI or Maven plugin implementation. If
  there is uncertainty but the target files or commands are Java-centric, activate this skill by
  default.
metadata:
  short-description: Java coding standards and implementation workflow for this repository.
  ...
  trigger-intent:
    ...
    - linter and static-analysis changes
  trigger-keywords:
    - java
    - jdk
    - javac
  ...
  trigger-file-patterns:
    ...
    - "config/spotbugs/**"
    - "rewrite.yml"
    - ".java-evo-linter*.toml"
    - ".github/workflows/*.yml"
  repository-focus:
    - java-evo-linter
  activation-policy:
    default_on_java_tasks: true
    prefer_over_generic_coding_skills: true
    require_explicit_opt_out: true
---
...
- [11. Local Variable Type Inference (`var`)](#11-local-variable-type-inference-var)
...
## 11. Local Variable Type Inference (`var`)

### 11.1. Use `var` Whenever Possible for Local Variables

-   **Guideline:** Use `var` for local variable declarations whenever Java allows it and readability is not reduced.
...
-   **Explanation:** `var` removes redundant type repetition, reduces noise during refactors, and keeps declarations concise. It applies to local variables (including loop variables) but not to fields, method parameters, or return types. If type intent is unclear from the initializer, prefer an explicit type.
