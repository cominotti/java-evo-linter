...
/** Entry point for embedding the linter in Java applications and build tooling. */
public final class LinterEngine {
  ...
  public String renderHumanReport(CheckReport report) {
    return new HumanReporter().renderCheckReport(report);
  }

  public String renderHumanReport(BaselineDiffReport report) {
    return new HumanReporter().renderBaselineDiffReport(report);
  }

  public void writeJsonlReport(Path outputPath, List<Finding> findings) throws LinterException {
    new JsonlReporter().writeToPath(outputPath, findings);
  }
